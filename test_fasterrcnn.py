...
def video_test():
    ...
    size = (width,height)
    result = cv2.VideoWriter("resultvideo.mp4",
        ...
        with torch.no_grad():
            ...
            for box , label , score in zip(boxes , labels , scores):
                if score > args.threshold:
                    ...
                    category = categories[label]
                    cv2.putText(frame , category , (int(xmin),int(ymin)) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (0,255,0) , 3 , cv2.LINE_AA)
                    cv2.putText(frame , str(round(score.item(),2)) , (int(xmin)+int(xmin),int(ymin)) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (0,255,0) , 3 , cv2.LINE_AA)
        result.write(frame)
